...
class BusinessStudiesPrompt extends StatefulWidget {
  const BusinessStudiesPrompt({super.key});

  @override
  State<BusinessStudiesPrompt> createState() => _BusinessStudiesPromptState();
}

class _BusinessStudiesPromptState extends State<BusinessStudiesPrompt> {
  ...
  void initState() {
    ...
    initFirebase().then((value) {
      ...
      _functionCallModel = FirebaseVertexAI.instance.generativeModel(
        ...
        model: 'gemini-1.5-flash',
        tools: [
          Tool(functionDeclarations: [businessStudiesControlTool]),
        ],
      );
  ...
  Future<Map<String, Object?>> learnBusinessStudies(
  ...
  final businessStudiesControlTool = FunctionDeclaration(
    'learnBusinessStudies',
    'Generate answers for the topic and in specified number of words.',
    Schema(SchemaType.object, properties: {
      'topic': Schema(SchemaType.string,
          description: 'Topic from Business Studies'),
...
Future<void> _testFunctionCalling(String message) async {
    ...
    const prompt = 'You are an expert Business Studies teacher. Please ask user about what user is interested to learn, how much user knows about the topic and based on the user response generate a learning plan to complete the topic also provide number of days it would be take complete the learning along with hours and then start teaching concepts step by step.';
    ...
    if (functionCalls.isNotEmpty) {
     ...
     final result = switch (functionCall.name) {
       ...
       'learnBusinessStudies' => await learnBusinessStudies(functionCall.args),
