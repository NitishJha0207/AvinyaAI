...
class BiologyPrompt extends StatefulWidget {
  const BiologyPrompt({super.key});

  @override
  State<BiologyPrompt> createState() => _BiologyPromptState();
}

class _BiologyPromptState extends State<BiologyPrompt> {
  ...
  void initState() {
    ...
    initFirebase().then((value) {
      ...
      _functionCallModel = FirebaseVertexAI.instance.generativeModel(
        ...
        model: 'gemini-1.5-flash',
        tools: [
          Tool(functionDeclarations: [biologyControlTool]),
        ],
      );
  ...
  Future<Map<String, Object?>> learnBiology(
...
Future<void> _testFunctionCalling(String message) async {
    ...
    const prompt = 'You are an expert Biology teacher. Please ask user about what user is interested to learn, how much user knows about the topic and based on the user response generate a learning plan to complete the topic also provide number of days it would be take complete the learning along with hours and then start teaching concepts step by step.';
    ...
    if (functionCalls.isNotEmpty) {
     ...
     final result = switch (functionCall.name) {
       ...
       'learnBiology' => await learnBiology(functionCall.args),
