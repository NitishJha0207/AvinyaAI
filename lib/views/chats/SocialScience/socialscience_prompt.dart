...
class SocialSciencePrompt extends StatefulWidget {
  const SocialSciencePrompt({super.key});

  @override
  State<SocialSciencePrompt> createState() => _SocialSciencePromptState();
}

class _SocialSciencePromptState extends State<SocialSciencePrompt> {
  ...
  void initState() {
    ...
    initFirebase().then((value) {
      ...
      _functionCallModel = FirebaseVertexAI.instance.generativeModel(
        ...
        model: 'gemini-1.5-flash',
        tools: [
          Tool(functionDeclarations: [socialScienceControlTool]),
        ],
      );
  ...
  Future<Map<String, Object?>> learnSocialScience(
  ...
  Future<void> _testFunctionCalling(String message) async {
    ...
    const prompt = 'You are an expert social science teacher. Please ask user about what user is interested to learn, how much user knows about the topic and based on the user response generate a learning plan to complete the topic also provide number of days it would be take complete the learning along with hours and then start teaching concepts step by step.';
    ...
    if (functionCalls.isNotEmpty) {
     ...
     final result = switch (functionCall.name) {
       ...
       'learnSocialScience' => await learnSocialScience(functionCall.args),
