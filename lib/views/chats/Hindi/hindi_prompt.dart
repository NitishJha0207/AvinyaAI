...
class HindiPrompt extends StatefulWidget {
  const HindiPrompt({super.key});

  @override
  State<HindiPrompt> createState() => _HindiPromptState();
}

class _HindiPromptState extends State<HindiPrompt> {
  ...
  void initState() {
    ...
    initFirebase().then((value) {
      ...
      _functionCallModel = FirebaseVertexAI.instance.generativeModel(
        ...
        model: 'gemini-1.5-flash',
        tools: [
          Tool(functionDeclarations: [hindiControlTool]),
        ],
      );
  ...
  Future<Map<String, Object?>> learnHindi(
...
Future<void> _testFunctionCalling(String message) async {
    ...
    const prompt = 'You are an expert Hindi language and literature teacher. Please ask user about what user is interested to learn, how much user knows about the topic and based on the user response generate a learning plan to complete the topic also provide number of days it would be take complete the learning along with hours and then start teaching concepts step by step.';
    ...
    if (functionCalls.isNotEmpty) {
     ...
     final result = switch (functionCall.name) {
       ...
       'learnHindi' => await learnHindi(functionCall.args),
