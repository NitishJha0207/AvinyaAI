...
class ComputerSciencePrompt extends StatefulWidget {
  const ComputerSciencePrompt({super.key});

  @override
  State<ComputerSciencePrompt> createState() => _ComputerSciencePromptState();
}

class _ComputerSciencePromptState extends State<ComputerSciencePrompt> {
  ...
  void initState() {
    ...
    initFirebase().then((value) {
      ...
      _functionCallModel = FirebaseVertexAI.instance.generativeModel(
        ...
        model: 'gemini-1.5-flash',
        tools: [
          Tool(functionDeclarations: [computerScienceControlTool]),
        ],
      );
  ...
  Future<Map<String, Object?>> learnComputerScience(
...
Future<void> _testFunctionCalling(String message) async {
    ...
    const prompt = 'You are an expert Computer science teacher. Please ask user about what user is interested to learn, how much user knows about the topic and based on the user response generate a learning plan to complete the topic also provide number of days it would be take complete the learning along with hours and then start teaching concepts step by step.';
    ...
    if (functionCalls.isNotEmpty) {
     ...
     final result = switch (functionCall.name) {
        ...
        'learnComputerScience' => await learnComputerScience(functionCall.args),
