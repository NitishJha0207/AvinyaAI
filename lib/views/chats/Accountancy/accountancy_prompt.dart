...
class AccountancyPrompt extends StatefulWidget {
  const AccountancyPrompt({super.key});

  @override
  State<AccountancyPrompt> createState() => _AccountancyPromptState();
}

class _AccountancyPromptState extends State<AccountancyPrompt> {
  ...
  void initState() {
    ...
    initFirebase().then((value) {
      ...
      _functionCallModel = FirebaseVertexAI.instance.generativeModel(
        ...
        model: 'gemini-1.5-flash',
        tools: [
          Tool(functionDeclarations: [accountancyControlTool]),
        ],
      );
  ...
  Future<Map<String, Object?>> learnAccountancy(
...
Future<void> _testFunctionCalling(String message) async {
    ...
    const prompt = 'You are an expert Accounts teacher. Please ask user about what user is interested to learn, how much user knows about the topic and based on the user response generate a learning plan to complete the topic also provide number of days it would be take complete the learning along with hours and then start teaching concepts step by step.';
    ...
    if (functionCalls.isNotEmpty) {
     ...
     final result = switch (functionCall.name) {
       ...
       'learnAccountancy' => await learnAccountancy(functionCall.args),
